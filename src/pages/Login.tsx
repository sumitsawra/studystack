...
export default function Login() {
  ...
  const { signInWithEmail, signInWithGoogle, loginAsDemo } = useAuthStore();
  ...
  const handleGoogle = async () => {
    ...
    catch (err: any) { addToast({ type: 'error', message: err.message || 'Google login failed' }); }
  };

  const handleDemo = () => { loginAsDemo(); addToast({ type: 'success', message: 'Welcome to demo mode!' }); navigate('/dashboard'); };
      ...
      <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="w-full max-w-md">
        ...
        <Card padding="lg">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <Input label="Password" type={showPassword ? 'text' : 'password'} placeholder="••••••••" value={password} onChange={(e) => setPassword(e.target.value)}
            ...
            <Button type="submit" size="lg" isLoading={loading} className="w-full">Sign In</Button>
          </form>

          <button onClick={handleDemo} className="w-full mt-3 py-2.5 text-sm text-secondary hover:text-primary transition-colors">
            Try Demo Mode
          </button>
        </Card>
